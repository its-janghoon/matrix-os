...
**This is history, not current state.** It is the chronological engineering
handoff kept from 2025 through September 2026, preserved because several
sections are the only written record of work that is hard to reconstruct: the
adversarial passes over consensus, the p2p layer and the WASM sandbox, the
verified timing side channels, and what to know before changing consensus.

Read it for those. Do NOT read it for what the system does today - many
sections preserve claims that were true, or merely proposed, at an earlier
commit, and the file says so about itself in several places. For the current
state and what to pick up next, see [`handoff.md`](../handoff.md) at the
repository root. For operating procedure, see [`runbooks/`](runbooks/).
