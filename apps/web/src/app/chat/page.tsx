...
function Chat() {
  ...
  // A budget this browser remembers but cannot yet act on, because no wallet is
  // connected. Held separately from `budget` so nothing tries to SPEND it.
  //
  // READ AT MOUNT, before any wallet connects. MetaMask needs an explicit
  // connect, so a reload starts with no owner to check a budget against - and
  // the card that names the budget and closes it was gated on that owner.
  // Refreshing therefore made an account holding the reader's money vanish from
  // the screen: precisely the "budget with a balance and no name" the runbook
  // warns about, produced by the page itself.
  //
  // A lazy initializer rather than an effect, because an effect that calls
  // setState in its body cascades a render for something that never changes
    ...
    <>
      ...
      <main className='min-h-screen bg-black px-4 pb-16 pt-24'>
        <div className={`${PAGE_COLUMN} space-y-6`}>
          ...
          {/*
            ABOVE the wallet card, because it is about money already committed
